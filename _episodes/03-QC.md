...
> > {{site.vm_prompt}}minimap2 -a -t 6 x map-hifi --secondary=no ./data/genome/kiwi_contig.fasta ./data/reads/hifi_reads.fastq > results/ref_hifi.sam
> > {{site.vm_prompt}}minimap2 -a -t 6 -x map-ont --secondary=no ./data/genome/kiwi_contig.fasta ./data/reads/ont_reads.fastq > results/ref_ont.sam
> > ~~~
> > {: .bash}
> {: .solution}
{: .challenge}
...
> > ~~~
> > {{site.vm_prompt}}grep -v "[0-9]\{2,\}" ref_hifi.sam > ref_hifi_filtered.sam
> > ~~~
> > {: .bash}
> {: .solution}
{: .challenge}
