...
> ## Reads
> The data we are using for this workshop are reads selected from a ~1.5M region in the kiwifruit genome. Assembling a full genome will take too long and a laptop is not a suitable device to assembly such a large genome. There are four read files:
~~~
...
And a 1.5MB region of our cultivar (assembled earlier):
~~~
...
> > ~~~
> > mkdir results
> > minimap2 -a -t 6 x map-hifi --secondary=no ./data/genome/kiwi_contig.fasta ./data/reads/hifi_reads.fastq > results/ref_hifi.sam
> > minimap2 -a -t 6 -x map-ont --secondary=no ./data/genome/kiwi_contig.fasta ./data/reads/ont_reads.fastq > results/ref_ont.sam
> > ~~~
> > {: .bash}
> {: .solution}
{: .challenge}
...
> > ## Solution
> > In our case we map reads from a genomics sample to only a part of the genome. Genomes contain many parts which are (very) similar, like gene copies, repeat regions, etc.
> > Reads which originate from these parts of the genome will map to our smaller region, usually with much less accuracy. 
> > When available, you should always use the whole genome to map reads from a genomics samples. Preferably including mtDNA and chloroplast.
...
> > In this case we can remove most false positives by selection reads with hardly any overlapping nucleotides (soft clippings):
> > ~~~
> > grep -v "[0-9]\{2,\}" ref_hifi.sam > ref_hifi_filtered.sam
> > ~~~
> > {: .bash}
> {: .solution}
{: .challenge}
